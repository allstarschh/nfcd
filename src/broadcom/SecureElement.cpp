...
bool SecureElement::deactivate(uint32_t seID)
{
  ...
  ALOGD("%s: enter; seID=0x%X, mActiveEeHandle=0x%X", __FUNCTION__, seID, mActiveEeHandle);
        ...
        if (mHciCfg.status == NFA_STATUS_OK) {
          for (UINT8 xx = 0; xx < mHciCfg.num_pipes; xx++) {
            if ((mHciCfg.pipe[xx].dest_host == destHost) && 
              ...
              ALOGD ("%s: found configured gate: 0x%02x  pipe: 0x%02x", __FUNCTION__, mNewSourceGate, mNewPipeId);
              break;
            }
          }
        }
...
bool SecureElement::disconnectEE(uint32_t seID)
{
  ...
  if (mUseOberthurWarmReset) {
    ...
    ALOGD("%s: try warm-reset on pipe id 0x%X; cmd=0x%X", __FUNCTION__, mNewPipeId, mOberthurWarmResetCommand);
    SyncEventGuard guard(mRegistryEvent);
    nfaStat = NFA_HciSetRegistry(mNfaHciHandle, mNewPipeId, 1, 1, &mOberthurWarmResetCommand);
    if (nfaStat == NFA_STATUS_OK) {
      mRegistryEvent.wait();
  ...
  case NFA_EE_MODE_SET_EVT:
    {
      ALOGD ("%s: NFA_EE_MODE_SET_EVT; status: 0x%04X  handle: 0x%04X  mActiveEeHandle: 0x%04X", 
             __FUNCTION__, eventData->mode_set.status, eventData->mode_set.ee_handle, sSecElem.mActiveEeHandle);
...
void SecureElement::nfaHciCallback(tNFA_HCI_EVT event, tNFA_HCI_EVT_DATA* eventData)
{
  ...
  case NFA_HCI_EVENT_SENT_EVT:
    {
      ALOGD("%s: NFA_HCI_EVENT_SENT_EVT; status=0x%X", __FUNCTION__, eventData->evt_sent.status);
    }
    break;
  ...
  case NFA_HCI_GET_REG_RSP_EVT:
    {
      ALOGD("%s: NFA_HCI_GET_REG_RSP_EVT; status: 0x%X; pipe: 0x%X, len: %d",
            __FUNCTION__, eventData->registry.status, eventData->registry.pipe, eventData->registry.data_len);
      if (eventData->registry.data_len >= 19 &&
          ((eventData->registry.pipe == STATIC_PIPE_0x70) || (eventData->registry.pipe == STATIC_PIPE_0x71))) {
  ...
  case NFA_HCI_EVENT_RCVD_EVT:
    {
      ALOGD("%s: NFA_HCI_EVENT_RCVD_EVT; code: 0x%X; pipe: 0x%X; data len: %u", 
            __FUNCTION__, eventData->rcvd_evt.evt_code, eventData->rcvd_evt.pipe, eventData->rcvd_evt.evt_len);
      if ((eventData->rcvd_evt.pipe == STATIC_PIPE_0x70) || (eventData->rcvd_evt.pipe == STATIC_PIPE_0x71)) {
        ...
        sSecElem.mActualResponseSize = 
          (eventData->rcvd_evt.evt_len > MAX_RESPONSE_SIZE) ? MAX_RESPONSE_SIZE : eventData->rcvd_evt.evt_len;
        ...
        if ((eventData->rcvd_evt.evt_len > 3) && (eventData->rcvd_evt.p_evt_buf[0] == 0x81)) {
          sSecElem.notifyTransactionListenersOfAid(
            &eventData->rcvd_evt.p_evt_buf[2], eventData->rcvd_evt.p_evt_buf[1]);
        }
      }
    }
    break;
